...
export function useCreateTask(teamId?: number | string) {
  const queryClient = useQueryClient();
  const key = ['tasks', teamId || 'all'];

  return useMutation({
    mutationFn: (data: TaskPayload) => axios.post('/api/tasks', data).then(r => r.data.task),
    onMutate: async (newTask) => {
      await queryClient.cancelQueries({ queryKey: key });
      const previous = queryClient.getQueryData(key);
      queryClient.setQueryData(key, (old: unknown[]) => [
        ...(old || []),
        { id: `temp-${Date.now()}`, ...newTask, status: newTask.status || 'todo', createdAt: new Date().toISOString() },
      ]);
      return { previous };
    },
...
export function useUpdateTask(teamId?: number | string) {
  const queryClient = useQueryClient();
  const key = ['tasks', teamId || 'all'];

  return useMutation({
    mutationFn: ({ id, ...data }: TaskPayload & { id: number }) =>
      axios.put(`/api/tasks/${id}`, data).then(r => r.data.task),
...
export function useDeleteTask(teamId?: number | string) {
  const queryClient = useQueryClient();
  const key = ['tasks', teamId || 'all'];
