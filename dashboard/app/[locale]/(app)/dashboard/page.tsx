...
// Constants
const LOCAL_STORAGE_KEY = 'psa-dashboard-grid-v2';
const GRID_COLS = 12;
const ROW_HEIGHT = 80;
...
export default function DashboardPage() {
  ...
  // Get active widget IDs from layouts
  const activeWidgetIds = useMemo(() => layouts.map((l) => l.i), [layouts]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed.layouts) && parsed.layouts.length > 0) {
        // Validate layouts
        const validLayouts = parsed.layouts.filter(
          (l: Layout) =>
            typeof l.i === 'string' &&
            typeof l.x === 'number' &&
            typeof l.y === 'number' &&
            typeof l.w === 'number' &&
            typeof l.h === 'number' &&
            AVAILABLE_WIDGETS.some((w) => w.id === l.i)
        );
        ...
        {isCustomizing && (
          <div
            ...
            style={{
              ...
              backgroundSize: `${(containerWidth - 32) / GRID_COLS}px ${ROW_HEIGHT}px`,
