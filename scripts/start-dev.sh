...
if [ ! -f dashboard/.env ]; then
  echo "⚠️  dashboard/.env not found. Copy from dashboard/.env.example and configure:"
...
echo "6. Dashboard (port 3001): cd dashboard && npm run dev"
echo ""
echo "Minimum for dashboard (projects/tasks/teams): 2, 3, 4, 5, 6"
echo "For login/signup: also start 1"
echo ""
echo "Ensure DATABASE_URL and JWT_SECRET are set in dashboard/.env"
echo ""
