...
export class AuthService {
  ...
  async checkAccountLockout(email: string, redis: RedisWrapper | null): Promise<{ locked: boolean; remainingTime?: number }> {
  ...
  async recordFailedLogin(email: string, redis: RedisWrapper | null): Promise<void> {
    if (!redis) return;
  ...
  async clearFailedLogins(email: string, redis: RedisWrapper | null): Promise<void> {
    if (!redis) return;
